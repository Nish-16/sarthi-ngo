...
import type { AboutSharedLeadershipContent, AboutSharedLeadershipPrinciple } from "@/types/content";
...
export default function SharedLeadershipForm({ initial }: { initial: AboutSharedLeadershipContent }) {
  const [data, setData] = useState<AboutSharedLeadershipContent>(initial);
  const [result, setResult] = useState<{ success?: boolean; error?: string } | null>(null);

  function set<K extends keyof AboutSharedLeadershipContent>(key: K, value: AboutSharedLeadershipContent[K]) {
    setData((d) => ({ ...d, [key]: value }));
  }

  return (
    <SectionShell title="Shared Leadership" description="CTA block with principles grid." onSave={async () => setResult(await saveAboutSharedLeadership(data))} saveResult={result}>
      <FormGroup title="Header & CTA">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="CTA Label"><Input value={data.cta} onChange={(v) => set("cta", v)} /></Field>
          <Field label="CTA Link"><Input value={data.ctaHref} onChange={(v) => set("ctaHref", v)} /></Field>
        </div>
      </FormGroup>
      ...
      <FormGroup title="Principles">
        <ArrayField<AboutSharedLeadershipPrinciple>
          ...
          createItem={() => ({ iconName: "Network", label: "", color: "bg-indigo-400/20 text-indigo-300 border-indigo-400/30" })}
