...
import type { AboutWhyYouthContent, AboutWhyYouthReason } from "@/types/content";
...
export default function WhyYouthForm({ initial }: { initial: AboutWhyYouthContent }) {
  const [data, setData] = useState<AboutWhyYouthContent>(initial);
  const [result, setResult] = useState<{ success?: boolean; error?: string } | null>(null);

  function set<K extends keyof AboutWhyYouthContent>(key: K, value: AboutWhyYouthContent[K]) {
    setData((d) => ({ ...d, [key]: value }));
  }

  return (
    <SectionShell title="Why Youth" description="Image + reasons section." onSave={async () => setResult(await saveAboutWhyYouth(data))} saveResult={result}>
      <FormGroup title="Header">
        ...
        <Field label="Description 1"><Input value={data.description1} onChange={(v) => set("description1", v)} /></Field>
        <Field label="Description 2"><Input value={data.description2} onChange={(v) => set("description2", v)} /></Field>
      </FormGroup>

      <FormGroup title="Image & Stat">
        <div className="grid grid-cols-2 gap-3">
          <Field label="Image URL"><Input value={data.image} onChange={(v) => set("image", v)} /></Field>
          <Field label="Image Alt"><Input value={data.imageAlt} onChange={(v) => set("imageAlt", v)} /></Field>
          <Field label="Stat Label"><Input value={data.statLabel} onChange={(v) => set("statLabel", v)} /></Field>
          <Field label="Stat Value"><Input value={data.statValue} onChange={(v) => set("statValue", v)} /></Field>
        </div>
      </FormGroup>
      ...
      <FormGroup title="Reason Pills">
        <ArrayField<AboutWhyYouthReason>
          ...
          createItem={() => ({ iconName: "Lightbulb", label: "", color: "bg-indigo-50 text-indigo-600 border-indigo-100" })}
          renderItem={(item, _i, onChange) => (
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Color Classes"><Input value={item.color} onChange={(v) => onChange({ ...item, color: v })} /></Field>
            </div>
          )}
        />
      </FormGroup>
