      ...
      <Container className="relative z-10">
        <div className="relative rounded-3xl bg-gradient-to-br from-indigo-700 via-indigo-600 to-cyan-600 text-white p-10 md:p-16 shadow-2xl shadow-indigo-300/40 overflow-hidden">
          ...
          <div className="relative z-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
            {content.stats.map(({ value, label, iconName }) => {
              const Icon = getIcon(iconName);
              return (
                <article
                  key={label}
                  className="rounded-2xl border backdrop-blur-md p-6 bg-white/15 border-white/20 hover:bg-white/20 transition-colors duration-300"
                >
                  <div className="w-10 h-10 rounded-xl bg-white/15 flex items-center justify-center mb-4">
                    <Icon className="w-5 h-5 text-white" />
                  </div>
                  <p className="text-4xl font-black tracking-tight">{value}</p>
                  <p className="mt-2 text-indigo-100 text-sm font-medium leading-snug">{label}</p>
                </article>
              );
            })}
          </div>
